  ...
  ({ className, size = 'default', children, ...props }, ref) => {
    ...
    return (
      <button
        ...
        data-size={size}
        className={Cn(
          'border-input focus-visible:border-ring focus-visible:ring-ring/50 dark:bg-input/30 dark:hover:bg-input/50 flex w-full items-center justify-between gap-2 rounded-md border bg-transparent px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50',
    ...
    <>
      ...
      {context?.open && (
        <div
          ...
          className={Cn(
            'bg-popover text-popover-foreground animate-in fade-in-0 zoom-in-95 absolute top-full left-0 z-50 mt-1 max-h-60 w-full min-w-[8rem] overflow-y-auto rounded-md border p-1 shadow-md',
            className
          )}
        >
          {children}
...
function SelectItem({ className, children, value, ...props }: SelectItemProps) {
  ...
  return (
    <div
      ...
      className={Cn(
        'hover:bg-accent hover:text-accent-foreground relative flex w-full cursor-pointer items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm select-none',
